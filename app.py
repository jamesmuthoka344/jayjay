...
def updateemployee():
    ...
    if request.method == 'POST':
        ...
        joining_date = request.form['joining_date']
        total_test_case = request.form['total_test_case']
        total_defects_found = request.form['total_defects_found']
        total_defects_pending = request.form['total_defects_pending']
        db = get_database()
        db.execute('update emp set name = ?, email =? , phone = ? , address = ?,total_projects = ?,joining_date= ?,total_test_case = ?,total_defects_found = ?,total_defects_pending = ? where empid = ?', [name, email, phone, address,total_projects,joining_date,total_test_case ,total_defects_found,total_defects_pending ,empid])
        db.commit()
        return redirect(url_for('dashboard'))
